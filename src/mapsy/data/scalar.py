...
class ScalarField(VolumetricField):
    """Scalar field on a Grid."""

    grid: Grid
    _integral: float | None
    ...
    def __array_finalize__(self, obj: np.ndarray | None) -> None:
        ...
        if isinstance(obj, ScalarField):
            self._integral = getattr(obj, "_integral", None)
    ...
    def integral(self) -> float:
        ...
        return float(self._integral)

    def _calc_integral(self) -> None:
        self._integral = float(np.sum(self)) * float(self.grid.volume) / float(self.grid.ndata)
    ...
    def tocube(
        ...
        with open(filename, "w") as cubefile:
            ...
            nat: int = len(atoms)
            cubefile.write(
                f"{nat:5d}{self.grid.origin[0]:12.6f}{self.grid.origin[1]:12.6f}{self.grid.origin[2]:12.6f}\n"
            )
            N1, N2, N3 = self.grid.scalars
            cubefile.write(
                f"{N1:5d}{self.grid.basis[0, 0]:12.6f}{self.grid.basis[0, 1]:12.6f}{self.grid.basis[0, 2]:12.6f}\n"
            )
            cubefile.write(
                f"{N2:5d}{self.grid.basis[1, 0]:12.6f}{self.grid.basis[1, 1]:12.6f}{self.grid.basis[1, 2]:12.6f}\n"
            )
            cubefile.write(
                f"{N3:5d}{self.grid.basis[2, 0]:12.6f}{self.grid.basis[2, 1]:12.6f}{self.grid.basis[2, 2]:12.6f}\n"
            )
            for mass, charge, pos in zip(
                ...
                cubefile.write(
                    f"{mass:5d}{charge:12.6f}{pos[0]:12.6f}{pos[1]:12.6f}{pos[2]:12.6f}\n"
                )
            count: int = 0
            for i in range(N1):
                for j in range(N2):
                    for k in range(N3):
                        if (i or j or k) and count % 6 == 0:
                            cubefile.write("\n")
                        cubefile.write(f" {self[i, j, k]: .5E}")
                        count += 1
    ...
    def toline(
        ...
        icenter = icenter - (self.grid.scalars * np.trunc(icenter // self.grid.scalars)).astype(
            "int"
        )
        if iaxis == 0:
            axis = self.grid.coordinates[0, :, icenter[1], icenter[2]]
            value = np.mean(self, (1, 2)) if planaraverage else self[:, icenter[1], icenter[2]]
        elif iaxis == 1:
            axis = self.grid.coordinates[1, icenter[0], :, icenter[2]]
            value = np.mean(self, (0, 2)) if planaraverage else self[icenter[0], :, icenter[2]]
        elif iaxis == 2:
            axis = self.grid.coordinates[2, icenter[0], icenter[1], :]
            value = np.mean(self, (0, 1)) if planaraverage else self[icenter[0], icenter[1], :]
        else:
            raise ValueError("Axis out of range")
        return axis, value

    def tocontour(
        self, center: npt.NDArray[np.float64], axis: int
    ...
    def plotprojections(
        ...
        _ = ax1.contourf(axis1_xz, axis2_xz, values_xz, levels=100, cmap=cmap, vmin=vmin, vmax=vmax)
        ax1.scatter(center[0], center[2])
        ax1.tick_params(labelbottom=False)
        ax1.set_ylabel("Z (a.u.)")

        ax3 = fig.add_subplot(gs[1, 0], sharex=ax1)
        _ = ax3.contourf(axis1_xy, axis2_xy, values_xy, levels=100, cmap=cmap, vmin=vmin, vmax=vmax)
