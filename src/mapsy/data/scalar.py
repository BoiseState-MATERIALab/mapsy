...
class ScalarField(VolumetricField):
    ...
    def tocube(
        ...
        with open(filename, "w") as cubefile:
            ...
            nat: int = len(atoms)
            # ✅ origin in Bohr
            cubefile.write(f"{nat:5d}{self.grid.origin[0]/Bohr:12.6f}{self.grid.origin[1]/Bohr:12.6f}{self.grid.origin[2]/Bohr:12.6f}\n")
            
            N1, N2, N3 = self.grid.scalars
            
            # ✅ basis vectors in Bohr
            cubefile.write(f"{N1:5d}{self.grid.basis[0,0]/Bohr:12.6f}{self.grid.basis[0,1]/Bohr:12.6f}{self.grid.basis[0,2]/Bohr:12.6f}\n")
            cubefile.write(f"{N2:5d}{self.grid.basis[1,0]/Bohr:12.6f}{self.grid.basis[1,1]/Bohr:12.6f}{self.grid.basis[1,2]/Bohr:12.6f}\n")
            cubefile.write(f"{N3:5d}{self.grid.basis[2,0]/Bohr:12.6f}{self.grid.basis[2,1]/Bohr:12.6f}{self.grid.basis[2,2]/Bohr:12.6f}\n")
            
            # ✅ atom positions in Bohr
            for mass, charge, pos in zip(atoms.get_atomic_numbers(), atoms.get_initial_charges(), atoms.positions):
                cubefile.write(f"{mass:5d}{charge:12.6f}{pos[0]/Bohr:12.6f}{pos[1]/Bohr:12.6f}{pos[2]/Bohr:12.6f}\n")
    ...
    def get_numpy_data(self, file):
        """
        Return (data, cell, origin) as np.ndarrays, no file.
        """
        with open(file, 'r') as cubeFile:
